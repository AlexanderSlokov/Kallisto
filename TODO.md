...
- **[URGENT] B-Tree Index Bypass (Phát hiện 07/03/2026)**: 
  - Server mode (HTTP/gRPC) hiện tại bypass hoàn toàn `BTreeIndex` (Mất đi lớp bảo vệ DoS O(log N)). 
  - CLI mode thì không nạp lại B-Tree từ RocksDB Disk lúc khởi động. 
  - **Action**: Viết RocksDB Iterator lúc startup để populate BTree; Wrap BTree validation vào API handlers. Đã lên lịch fix vào Chủ Nhật.
