 ...
 * - Load factor = 50% (items_per_shard / 8 buckets)
 * - Option A: Each shard owns independent CuckooTable (avoids cross-shard kick deadlock)
 */
...
public:
    ...
    size_t get_shard_index(const std::string& key) const {
        ...
        return SipHash::hash(key, 0xDEADBEEF64, 0xCAFEBABE64) & (NUM_SHARDS - 1);
    }
...
private:
    ...
    inline CuckooTable* getShard(const std::string& key) const {
        // Use same seed as CuckooTable::hash_1_full for consistent distribution
        size_t idx = SipHash::hash(key, 0xDEADBEEF64, 0xCAFEBABE64) & (NUM_SHARDS - 1);
