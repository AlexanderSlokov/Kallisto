...
public:
  ...
  size_t capacity;
  const int max_displacements_ = 100;
  ...
  size_t hash_2(const std::string& key) const {
    return (bad_hash(key) + 1) % capacity;
  } // Slightly different to avoid instant fail, but still weak
...
void run_flooding_test() {
  ...
  // A. WEAK SYSTEM
  {
    ...
    for (const auto& k : attack_keys) {
      if (weak_table.insert(k))
        success++;
    }
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> elapsed = end - start;

    std::cout << "  > WEAK Hash (Simulated): " << elapsed.count() << " ms | Success: " << success
              << "/" << N << "\n";
    if (success < N)
      std::cout << "    (Many collisions caused insertion failure/cycles)\n";
  }
  ...
  // B. REAL SYSTEM (Kallisto)
  {
    ...
    for (const auto& k : attack_keys) {
      ...
      entry.key = k;
      if (real_table.insert(k, entry))
        success++;
    }
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> elapsed = end - start;

    std::cout << "  > SIPHASH (Kallisto)   : " << elapsed.count() << " ms | Success: " << success
              << "/" << N << "\n";
  }
}
...
void run_btree_gate_test() {
  ...
  for (int i = 0; i < 100; ++i)
    btree.insertPath("/valid/path/" + std::to_string(i));

  // Attack: 10,000 requests to INVALID paths
  std::vector<std::string> invalid_paths;
  for (int i = 0; i < N; ++i)
    invalid_paths.push_back("/hack/attempt/" + std::to_string(i));

  auto start = std::chrono::high_resolution_clock::now();
  int blocked = 0;
  for (const auto& p : invalid_paths) {
    if (!btree.validatePath(p))
      blocked++;
  }
  auto end = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double, std::milli> elapsed = end - start;
